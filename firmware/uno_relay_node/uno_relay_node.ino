...
// Dinamik Histerezis (BLE'den gelecek)
...
BLECharacteristic chMode(UUID_SYS_MODE, BLEWrite | BLEWriteWithoutResponse, 1);
BLECharacteristic chHysteresis(UUID_HYSTERESIS, BLEWrite | BLEWriteWithoutResponse, 4);
...
static uint8_t system_mode = 0; // 0: Heating, 1: Cooling, 2: Fan, 3: Off
...
void setup() {
  ...
  svc.addCharacteristic(chMode);
  svc.addCharacteristic(chHysteresis);
...
void loop() {
  ...
  while (central.connected()) {
    ...
    // GIGA Histerezis yazdı mı?
    if (chHysteresis.written()) {
      float v;
      if (readFloat4(chHysteresis, v)) hysteresis = v;
      Serial.print("Hysteresis changed to: ");
    ...
    if (isfinite(temp_C) && isfinite(setpoint_C)) {
      ...
      if (system_mode == 3) { // Off
        shouldOn  = false;
        shouldOff = true;
      } else if (system_mode == 0) { // Heating
        shouldOn  = (temp_C <= (setpoint_C - hysteresis));
        shouldOff = (temp_C >= (setpoint_C + hysteresis));
      } else if (system_mode == 1) { // Cooling
        shouldOn  = (temp_C >= (setpoint_C + hysteresis));
        shouldOff = (temp_C <= (setpoint_C - hysteresis));
      } else if (system_mode == 2) { // Fan
        shouldOn  = true; // Fan modu, röle sürekli açık
        shouldOff = false;
      }
      ...
      Serial.print("  MOD="); Serial.print(system_mode);
